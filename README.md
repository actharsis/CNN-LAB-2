...
## Анализ результатов
Первая попытка обучения нейросети оказалась неудачной: максимальная точность по метрике качества ~35%. Для второй попытки была изменена архитектура сети, в частности были использованы предобученные веса. В результате получились гораздо более качественные результаты: точность по метрике качества возрасла больше чем на 40%, также можно отметить более быстрое обучение. В технике Transfer learning можно выделить следующие плюсы: повышение точности классификации в случае недостатка данных и повышенную скорость обучения
